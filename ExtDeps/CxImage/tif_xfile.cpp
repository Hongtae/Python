...
// modified: to resolve conflict with libtiff  by tiff2766@gmail.com, 2013-05-21
TIFF* CX_TIFFOpen(CxFile* stream, const char* mode)
{
	_TIFFerrorHandler = NULL;	// added to ignore error message. by tiff2766@gmail.com, 2013-11-08
	TIFF* tif = TIFFClientOpen("IMAGE.TIF", mode,
	    (thandle_t)stream,
	    _tiffReadProcEx, _tiffWriteProcEx, _tiffSeekProcEx, _tiffCloseProcEx,
	    _tiffSizeProcEx, _tiffMapProcEx, _tiffUnmapProcEx);
	if (tif)
