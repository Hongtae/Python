 ...
 */
bool CxImage::Save(const CXCHAR * filename, uint32_t imagetype)
{
	FILE* hFile;	//file handle to write the image

	if ((hFile=CXFopen(filename,CXSTR("wb")))==NULL)  return false;
...
////////////////////////////////////////////////////////////////////////////////
/**
 * exports the image into a RGB buffer, Useful for OpenGL applications.
 * \param buffer: output memory buffer pointer. Must be NULL,
 * the function allocates and fill the memory,
 * the application must free the buffer, see also FreeMemory().
 * \param size: output memory buffer size.
 ...
 * \return true if everything is ok
 */
bool CxImage::Encode2RGB(uint8_t * &buffer, int32_t &size, bool bFlipY)
{
	if (buffer!=NULL){
		strcpy(info.szLastError,"the buffer must be empty");
		return false;
...
// For UNICODE support: char -> CXCHAR
CxImage::CxImage(const CXCHAR * filename, uint32_t imagetype)
{
	Startup(imagetype);
...
bool CxImage::GetExifThumbnail(const CXCHAR *filename, const CXCHAR *outname, int32_t type)
{
	switch (type){
#if CXIMAGE_SUPPORT_RAW
