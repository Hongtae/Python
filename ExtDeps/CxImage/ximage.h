...
typedef struct tagCxTextInfo
{
	...
	CXCHAR    text[256];  ///< text for windows CE
#else
	CXCHAR    text[4096]; ///< text (char -> CXCHAR for UNICODE [Cesar M])
#endif
...
/** \addtogroup Constructors */ //@{
	...
	CxImage(const CXCHAR * filename, uint32_t imagetype);	// For UNICODE support: char -> CXCHAR
...
/** \addtogroup Attributes */ //@{
	...
	bool  GetExifThumbnail(const CXCHAR *filename, const CXCHAR *outname, int32_t imageType);
	...
	static uint32_t GetTypeIdFromName(const CXCHAR* ext);
...
/** \addtogroup Painting */ //@{
	...
	int32_t	DrawString(HDC hdc, int32_t x, int32_t y, const CXCHAR* text, RGBQUAD color, const CXCHAR* font, int32_t lSize=0, int32_t lWeight=400, uint8_t bItalic=0, uint8_t bUnderline=0, bool bSetAlpha=false);
	// <VATI> extensions
...
#endif
	// For UNICODE support: char -> CXCHAR
	bool Load(const CXCHAR* filename, uint32_t imagetype=0);
...
/** \addtogroup Encode */ //@{
...
#endif
	// For UNICODE support: char -> CXCHAR
	bool Save(const CXCHAR* filename, uint32_t imagetype);
	...
	bool Encode2RGB(CxFile *hFile, bool bFlipY = false);
	bool Encode2RGB(uint8_t * &buffer, int32_t &size, bool bFlipY = false);
//@}
