...
bool CxImageRAW::GetExifThumbnail(const CXCHAR *filename, const CXCHAR *outname, int32_t type)
{	
	DCRAW  dcr;

	CxIOFile file;
	if (!file.Open(filename, CXSTR("rb"))) 
		return false;
  ...
  cx_try
  {
	...
	{
 		FILE* file = CXFopen(outname, CXSTR("wb"));
		DCRAW* p = &dcr;
