...
int32_t CxImage::DrawString(HDC hdc, int32_t x, int32_t y, const CXCHAR* text, RGBQUAD color, const CXCHAR* font, int32_t lSize, int32_t lWeight, uint8_t bItalic, uint8_t bUnderline, bool bSetAlpha)
{
	if (IsValid()){
		//get the background
		HDC pDC;
...
void CxImage::InitTextInfo( CXTEXTINFO *txt )
{
    ...
    _tcscpy( txt->lfont.lfFaceName, CXSTR("Arial")); //use CXCHAR mappings <Cesar M>
    ...
    // the text 
    _tcscpy( txt->text, CXSTR("Sample Text")); // text use CXCHAR mappings <Cesar M>
    txt->align = DT_CENTER;
    return;
}
